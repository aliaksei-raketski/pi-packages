...
function parseConfig(
  ...
  if (candidate.layout !== undefined) {
    ...
    if (normalized === null) {
      diagnostics.push(
        `Ignoring invalid ${source} layout: expected array of strings or nested arrays.`,
      );
    } else {
  ...
  if (candidate.colors !== undefined) {
    ...
    if (colors) {
      ...
      diagnostics.push(
        `Ignoring invalid ${source} colors: expected string or string/number state maps.`,
      );
    }
  }
...
function mergeConfig(
  ...
  patch: Partial<StatuslineConfig> | null,
): StatuslineConfig {
  if (!patch) {
    return { ...base };
  }
  return {
    ...
    colors: patch.colors ? mergeColorMaps(base.colors, patch.colors) : { ...base.colors },
  };
}
