...
export function renderLayoutLine(
  ...
  const totalContentWidth = visibleSegments.reduce(
    (sum, segment) => sum + visibleWidth(segment),
    0,
  );
...
export function renderLayoutLines(
  ...
  const shouldFallback = layout.some(
    (line) => getLineContentWidth(line, itemText, separator) > width,
  );
  if (shouldFallback) {
